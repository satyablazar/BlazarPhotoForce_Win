...
namespace PhotoForce.OrdersManagement
{
    public class ImportOrdersProgressBarViewModel : ViewModelBase
    {
        ...
        #region Constructor
        public ImportOrdersProgressBarViewModel(string folder, bool isFromExcel,bool isGotPhotoChecked, DataTable excelData)
        {
            ...
            processOrdersItems(folder, isFromExcel,isGotPhotoChecked);
        }
        #endregion
        ...
        /// </summary>
        async void processOrdersItems(string folder, bool isFromExcel, bool isGotPhotoChecked )
        {
            ...
            else if (!string.IsNullOrEmpty(folder) && isFromExcel)
            {
                ...
                else
                {
                    ...
                    await importOrders3(isGotPhotoChecked);
                }
            }
            else
            {
        ...
        /// </summary>
        /// <returns></returns>
        async Task importOrders3(bool isGotPhotoChecked)
        {
            ...
            await Task.Run(() =>
            {
                foreach (DataRow dr in dataTableFromExcel.Rows)
                {
                    ...
                    try
                    {
                        ...
                        else // if GotPhotoChecked
                        {
                            #region GotPhoto Orders Import
                            if (dr["OrdersPhoto.original Filename"].ToString() == "Order.jpg") { continue; }
                            bool GotPhotoOrderExist = clsOrders.getOrderDetails(db, Convert.ToInt32(dr["Order.orderkey"]));
                            using (TransactionScope ts = new TransactionScope())
                            {
                                using (PhotoSorterDBModelDataContext dbb = new PhotoSorterDBModelDataContext(clsConnectionString.connectionString))
                                {
                                    #region Condition A
                                    if (!GotPhotoOrderExist)
                                    {
                                        xmlFilesCount++;
                                        isNewOrder = true; isM134SubExists = false; isM134Exists = false;
                                        ...
                                        previousOrderNumber = Convert.ToInt32(dr["Order.orderkey"]);
                                        ...
                                        _objOrder.OrdersImportId = orderImportBatchId == 0 ? null : orderImportBatchId;
                                        //_objOrder.VendorDate = dr["creationDate"] == null ? null : (DateTime?)Convert.ToDateTime(dr["creationDate"], new System.Globalization.CultureInfo("en-US")); //No-Column in Excel //Mohan
                                        _objOrder.SimplePhotoOrderID = Convert.ToInt32(dr["Order.orderkey"]);
                                        simplePhotoOrderId = _objOrder.SimplePhotoOrderID;
                                        //_objOrder.OrderTotal = Convert.ToDecimal(dr["orderTotal"]);//No-Column in Excel //Mohan
                                        //_objOrder.ShippingCost = Convert.ToDecimal(dr["amount"]);//No-Column in Excel //Mohan
                                        //_objOrder.OrderType = dr["type"].ToString();  //No-Column in Excel //Mohan
                                        ...
                                        //Customer Information
                                        _objOrder.Cust_FirstName = dr["Photographer.firstname"].ToString();
                                        _objOrder.Cust_LastName = dr["Photographer.lastname"].ToString();
                                        // commented by hema bcz no emial field in attached excel file
                                        // _objOrder.Cust_Email = dr["Email"].ToString();

                                        //No-Column in Excel //Mohan
                                        //Commented by hema for card (https://trello.com/c/BLKdetmm/914-here-is-the-sample-spreadsheet-that-will-be-downloaded-from-gotphoto-to-be-imported-into-photoforce-orders)
                                        //_objOrder.Cust_Address = dr["Address"].ToString();
                                        //_objOrder.Cust_City = dr["City"].ToString();
                                        //_objOrder.Cust_State = dr["State"].ToString();
                                        //_objOrder.Cust_PostalCode = dr["Zip"].ToString();
                                        ...
                                        //Shipping Information
                                        _objOrder.Ship_FirstName = dr["ShippingAddress.firstname"].ToString();
                                        _objOrder.Ship_LastName = dr["ShippingAddress.lastname"].ToString();
                                        _objOrder.Ship_Address = dr["Address"].ToString();
                                        // _objOrder.Ship_City = dr["City"].ToString();
                                        _objOrder.Ship_State = dr["ShippingAddress.state"].ToString();
                                        _objOrder.Ship_PostalCode = dr["ShippingAddress.zip"].ToString();
                                        _objOrder.Ship_Country = dr["ShippingAddress.country"].ToString();
                                        //_objOrder.Ship_UseBillToShip = dr["useBilltoShip"].ToString();
                                        //_objOrder.Ship_Phone = dr["phone"].ToString();
                                    ...
                                    //for (int i = 0; i < galleryInfoList.Count; i++)
                                    //{
                                    if (tempOrderId != 0 && previousOrderNumber == Convert.ToInt32(dr["Order.orderkey"]))
                                    {
                                        #region New Code (Loop Through Gallery Info)
                                        if (isProgressCancelled)
                                        {
                                        ...
                                        int? classPhoto = null;
                                        //based on the image name get all the details
                                        string imageName = dr["OrdersPhoto.original Filename"].ToString();
                                        string billingCode = "";
                                        ...
                                        StudentPhotoOrder _objStudentPhotoOrder = new StudentPhotoOrder();
                                        //Commented by hema (there is no such field in excel )
                                        //string tempPackage = dr["packages"].ToString();                                    
                                        //if (!string.IsNullOrEmpty(tempPackage))
                                        //{
                                        //    db = new PhotoSorterDBModelDataContext(clsConnectionString.connectionString);
                                        ...
                                        //}
                                        //else
                                        billingCode = dr["OrdersPhoto.sku"].ToString();
                                        if (imageDetails == null)
                                        {
                                            //get the order tempOrderId
                                            Order tempOrderDetail = clsOrders.getOrderById(dbb, tempOrderId);
                                        ...
                                        _objStudentPhotoOrder.GroupImageId = classPhoto;
                                        _objStudentPhotoOrder.sp_SimplePhotoBillingCode = billingCode;
                                        _objStudentPhotoOrder.sp_ItemDescription = dr["OrdersPhoto.product Name"].ToString();
                                        _objStudentPhotoOrder.Quantity = Convert.ToInt32(dr["OrdersPhoto.count"]);

                                        _objStudentPhotoOrder.gp_FileName = dr["OrdersPhoto.filename"].ToString();
                                        _objStudentPhotoOrder.gp_FileName2 = dr["OrdersPhoto.filename2"].ToString();
                                    ...
                                    {
                                        isOrdersAlreadyExist = true;
                                        alreadyExistedOrders = alreadyExistedOrders + Convert.ToInt32(dr["Order.orderkey"]) + ", ";
                                    }
                                    #endregion // Condition B
                                }
                                ts.Complete();
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (!isGotPhotoChecked)
                            ...
                            MVVMMessageService.ShowMessage("Error while processing " + dr["Order.orderkey"] + "\n\n" + ex.Message);
                        clsStatic.WriteExceptionLogXML(ex);
                    }
                }
            });
